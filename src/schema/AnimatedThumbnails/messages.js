export const ps = (type, attempt, action) => {
  switch (type) {
    ...
    case 'already-generated':
      return {
        type,
        message: 'Animated thumbnails already exist for this video. Delete them to regenerate.',
      }
    ...
    case 'loading':
      ...
      if (attempt >= 20) {
        return {
          type,
          message: "Vimeo is taking longer than usual. We'll keep checking — don't close this tab.",
        }
      }
      if (attempt >= 8) {
        return {
          type,
          message: "Still generating — Vimeo's animated thumbnail builds usually take 1–3 minutes.",
        }
      }
      if (attempt >= 1) {
        return {
